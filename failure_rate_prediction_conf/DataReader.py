...
from failure_rate_prediction_conf.data_entry_class.ImpressionEntry import MIN_OCCURRENCE_SYMBOL, \
    MIN_OCCURRENCE as ORIGIN_MIN_OCCURRENCE
...
class SurvivalData:
    ...
    def __init__(self, times, events, sparse_features, sparse_headerbids,
        ...
        if only_hb_imp:
            ...
            self.times, self.events, self.sparse_features, self.sparse_headerbids = \
                self.times[hb_imp_rows], self.events[hb_imp_rows], self.sparse_features[hb_imp_rows], \
                self.sparse_headerbids[hb_imp_rows]
    ...
    def make_sparse_batch(self, batch_size=10000, only_freq=False):
        ...
        if only_freq:
            ...
            assert self.times.shape[0] == self.events.shape[0] == self.sparse_features.shape[0] == \
                   self.sparse_headerbids.shape[0]
        ...
        while start_index < self.num_instances:
            ...
            filter_mask = [np.array(list(map(lambda i: i not in infreq_col_set, indices_arr))).astype(bool) for
                           indices_arr in feat_indices_batch]
            ...
            feat_values_batch = pad_sequences(feat_values_batch, maxlen=self.max_nonzero_len, padding='post', value=0.0,
                                              dtype='float32')
...
if __name__ == "__main__":
    ...
    for t, e, f_ind, f_val, h_ind, h_val, max_nonzero_len in s.make_sparse_batch(5, only_freq=False):
        # print(t)
        # print(e)
        print(f_ind)
        print(f_val)
