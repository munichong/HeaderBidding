...
HEADER_BIDDING_KEYS = ('mnetbidprice', # Count = 4216971 (40.93%), Mean Value = 1.83 | Count = 4968367 (85.06%), Mean Value = 3.58
...
for COLNAME in ['NetworkBackfillImpressions', 'NetworkImpressions']:
    ...
    for hb_agent, count, value in sorted(zip(HEADER_BIDDING_KEYS, total_count, values), key = lambda x:x[1], reverse=True):
        print('\t%s: Count = %d (%.2f%%), Mean Value = %.2f' % (hb_agent, count, count / total_entries * 100, value / (count + 1e-6)))
